...
FROM_PWD = "yyvubeixvfctpsea" #Carmen from UAM-Linux
...
# Look at all emails from Italki
IT_mails_data = []
for idx in range(n_messages-1,n_messages-N-1,-1):
    id_mail = int(id_list[idx])
    Subject, From, body = mail_reader(mail,id_mail)
    IT_mails_data.append(request_info_IT(body))
    
column_names = ["Name","StartDate","EndDate","Duration","Id"]
dfNewIT = pd.DataFrame(IT_mails_data, columns=column_names)
